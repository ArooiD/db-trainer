...
Файловое пространство — постоянная правая панель `artifacts-dock` внутри
`app-shell`, рядом с контентом (не модалка). Открыта по умолчанию, сворачивается
кнопкой ◆ в шапке и «›» в заголовке панели; состояние сохраняется в localStorage
(ключ `it-study-lab.artifacts-dock`). Компонент `ArtifactsView.jsx`, состояние
`artifactsOpen` и `toggleArtifacts` в `app-store.jsx`.
