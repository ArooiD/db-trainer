...
Кнопка «🛠 Спроектировать» открывает полноэкранный canvas-редактор схемы. В нём
по заданию можно набросовать структуру базы до написания запроса:

- создать таблицу, переименовать её, удалить;
- добавлять колонки, выбирать тип (`INTEGER`, `TEXT`, `REAL`, `DATE`, `BOOLEAN`),
  помечать `PRIMARY KEY`;
- проводить связь: тянуть мышкой от зелёной точки справа от колонки на другую
  таблицу — добавляется колонка-внешний ключ и линия связи;
- таскать таблицы, двигать фон (ЛКМ по пустому месту), масштабировать колесом;
- справа сразу виден готовый `CREATE TABLE` по всем таблицам (с учётом порядка
  зависимостей) — его можно скопировать или перенести в свободный режим;
- набросок автосохраняется отдельно для каждой задачи в `localStorage`, его
  можно выгрузить в JSON и загрузить обратно.
