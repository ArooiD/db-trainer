...
Также доступен **Programming Sandbox**: JavaScript исполняется в браузере, а Python запускается через локально собранный Pyodide runtime. Python загружается только при первом выборе, поэтому Database Sandbox не получает лишний вес.
...
- работать в интерфейсе браузерной SQL IDE;
- открывать таблицы из дерева объектов двойным кликом;
- просматривать результат в полноразмерной табличной сетке;
- перемещать рабочую область ERD и перетаскивать таблицы диаграммы;
...
Отдельная вкладка объединяет теорию с переходом к практической среде. Каталог уже содержит четыре направления:

- «Базы данных» — первый наполненный курс;
- «Программирование IT-систем» — наполненный курс из пяти лекций: выполнение программ, алгоритмы, Python, JavaScript и архитектура;
...
`src/` — React-приложение и точка входа Vite. `web/` хранит учебные runtime и legacy-модули, которые подключаются через React shell в процессе поэтапного переноса интерфейса. `dist/` и `dist-standalone/` — генерируемые результаты сборки и руками не редактируются.
...
После этого откройте адрес, который покажет Vite (по умолчанию `http://localhost:5173`). Работает hot reload: изменения в React-коде сразу появляются в браузере.
