// Фронтенд SQL-тренажёра. Задачи берутся из window.DB_TASKS (js/tasks.js),
// выполнение SQL делегируется серверу (POST /api/execute).
...
function normalizeCell(v) {
  ...
  if (typeof v === "number") {
    // 150000 и 150000.0 считаются равными; дробные округляем от погрешности
...
function resultsMatch(a, b, ordered) {
  ...
  // без учёта порядка: мультимножество строк
  const mb = {};
...
function renderTable(result) {
  ...
  if (result.error)
    return `<div class="empty" style="color:var(--danger)">${escapeHtml(
      result.error
    )}</div>`;
...
function enableSandbox(sqlDraft) {
  sandboxMode = true;
  currentTask = null;
  ...
  $("editor").value = sqlDraft || "SELECT * FROM departments;";
...
async function run() {
  ...
  try {
    ...
    fb.className = "feedback no";
    fb.innerHTML = "Ошибка соединения с сервером: " + escapeHtml(String(e));
  } finally {
...
async function showSchema() {
  ...
  openModal(`<h3>Схема учебной базы данных</h3><pre>${escapeHtml(
    data.schema
  )}</pre><p style="color:var(--muted)">Пример запроса: <code>SELECT * FROM employees LIMIT 5;</code></p>`);
}
...
$("btn-hint").addEventListener("click", () => {
  ...
  const box = $("hint-box");
  box.textContent = "💡 " + currentTask.hint;
